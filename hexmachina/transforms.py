...
def convert_to_euler(R):
    alpha, beta, gamma = 0
    if np.abs(R[3,1]) != 1:
        beta = - np.arcsin(R[3,1])
        alpha = np.arctan2(R[3,2] / np.cos(beta), R[3,3] / np.cos(beta))
        gamma = np.arctan2(R[2,1] / np.cos(beta), R[1,1] / np.cos(beta))
    else:
        gamma = 0
        if R[3,1] == -1:
            beta = np.pi / 2
            alpha = gamma + np.arctan2(R[1,2], R[1,3])
        else:
            beta = - np.pi / 2
            alpha = - gamma + np.arctan2(-R[1,2], -R[1,3])
